...
from file_handler import get_relative_path, get_files, get_current_path
...
class GUI:
  def __init__(self, title, geometry='1000x500'):
    ...
    dropdown_labels = ['Count cell size by', 'Blurring', 'Operation choice']
    ...
    'count_cell_size_by':['area', 'width'], 'blurring': ['yes', 'no'],
    ...
    execution_button = Button(self.root, text="Run", command=self.output_image)
  ...
  def output_image(self):
    ...
    if len(output_images) != 0:
      ...
      image_label = Label(self.root, image=PhotoImage(file=example_output_image))
      image_label.grid(row=1,column=5)
